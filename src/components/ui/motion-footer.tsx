...
.footer-aurora {
  background: radial-gradient(
    ...
    color-mix(in oklch, #a855f7 15%, transparent) 40%, 
    transparent 70%
  );
}
...
.footer-giant-bg-text {
  ...
  background: linear-gradient(180deg, color-mix(in oklch, #ffffff 20%, transparent) 0%, transparent 65%);
...
const MarqueeItem = () => (
  <div className="flex items-center space-x-12 px-6">
    <span>Interactive Learning</span> <span className="text-primary/60">✦</span>
    <span>Expert Instructors</span> <span className="text-secondary/60">✦</span>
    <span>Skill Development</span> <span className="text-primary/60">✦</span>
    <span>Community Support</span> <span className="text-secondary/60">✦</span>
    <span>Lifelong Education</span> <span className="text-primary/60">✦</span>
  </div>
);
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        ...
        isMobile ? { x: 50, y: 0, opacity: 0 } : { y: 50, x: 0, opacity: 0 },
        {
          x: 0,
          y: 0,
          opacity: 1,
  ...
  return (
    <>
      ...
      */}
      <div
        ref={wrapperRef}
        className="relative h-[60dvh] md:h-[100dvh] w-full mt-8 md:mt-24"
        style={{ clipPath: "polygon(0% 0, 100% 0%, 100% 100%, 0 100%)" }}
      >
        {/* The actual footer stays fixed to the viewport underneath everything */}
        <footer className="fixed bottom-0 left-0 flex h-[60dvh] md:h-[100dvh] w-full flex-col justify-between overflow-hidden bg-black text-white cinematic-footer-wrapper">
          ...
          <div className="absolute top-4 md:top-36 left-0 w-full overflow-hidden border-y border-border/50 bg-background/60 backdrop-blur-md py-3 md:py-4 z-10 -rotate-2 scale-110 shadow-2xl">
            <div className="flex w-max animate-footer-scroll-marquee text-[10px] md:text-sm font-bold tracking-[0.2em] md:tracking-[0.3em] text-muted-foreground uppercase">
              <MarqueeItem />
              <MarqueeItem />
            </div>
          </div>
          ...
          <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 mt-16 md:mt-20 w-full max-w-5xl mx-auto">
            <h2
              ref={headingRef}
              className="text-5xl md:text-8xl font-black footer-text-glow tracking-tighter mb-8 md:mb-12 pb-2 md:pb-4 text-center"
            >
